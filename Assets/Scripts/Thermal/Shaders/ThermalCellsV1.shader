...
// Summary: renders V1 thermal cells with a position (Vector3) and temperature (float)
...
CGINCLUDE
	...
	v2f vert(uint id : SV_VertexID)
	{
		...
		v2f o;
		o.pos = float4(data.position.x, data.position.y, data.position.z, 1);
...
Shader "Thermal/ThermalCellsV1"
{
	Properties
	{
		Resolution("Resolution", int) = 1
		HalfSize("Half Size", float) = 0.1
		HalfHeight("Half Height", float) = 0.1
		MinTemperature("Min Temperature", float) = 0.0
